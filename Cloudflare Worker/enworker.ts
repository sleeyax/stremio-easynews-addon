...
async function getItemInfoFromOmdb(imdbId: string): Promise<ItemInfo> {
...
async function searchEasynews(title: string, year: string): Promise<EasynewsResult[]> {
...
async function searchEasynewsSeries(title: string, year: string, season: string, episode?: string): Promise<EasynewsResult[]> {
  let searchTerm;
  if (season && episode) {
...
function parseEasynewsSearchResults(html: string): EasynewsResult[] {
